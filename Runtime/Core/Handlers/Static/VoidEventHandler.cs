...
{
    internal class VoidEventHandler : EventHandlerBase<IEvent, string>
    {
        private static readonly object SubscriptionsLock = new();

        public SubscriptionResult Subscribe(string key, Action action)
        {
            ...
            lock (SubscriptionsLock)
            {
                ...
                var subscription = new VoidSubscription(action, token);
        ...
        public void Publish(string key)
        {
            ...
            for (var index = 0; index < allSubscriptions.Count; index++)
            {
                ...
                try
                {
                    var voidSubscription = subscription as VoidSubscription;
                    voidSubscription?.Publish();
                }
                catch (Exception)
                {
        ...
        internal VoidEventHandler(ISubscriptionContainer<ISubscription, string> container,
    ...
    internal class VoidSubscriptionContainer : SubscriptionContainer<string>
    {
    }
}